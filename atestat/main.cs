    ...
    public partial class main : Form
    {
        ...
        private void cereriprietenie_Click(object sender, EventArgs e)
        {
            ...
            if (f.exista_cereri_active(f.getusername(id)))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT username,nume,prenume FROM users WHERE username " +
                    "IN (SELECT username1 FROM requests WHERE username2 = '" + f.getusername(id) + "')", db.con);
